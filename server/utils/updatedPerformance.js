...
module.exports.updatedExtraPerformance = (peu, roundNum, netID, doc) => {
    // console.log("extra performance - ", peu);
    ...
    if (roundNum == 1) {
        switch (peu.game) {
            case 1:
                ...
                if (peu.extra || peu.extra === 0) game1.point = basePoint + peu.extra;
                // CHECK WITH A CONDITION - IF CURRENT NOG IS MORE THAN PREVIOUS RETURN WITH NOG OR WITHOUT NOG / ONLY GAME
                return { game1 }
            case 2:
                let game2 = new Object();
                ...
                // SET OR CHANGE VALUE FROM USER INPUT 
                if (peu.extra || peu.extra === 0) game2.point = basePoint + peu.extra;
                // CHECK WITH A CONDITION - IF CURRENT NOG IS MORE THAN PREVIOUS RETURN WITH NOG OR WITHOUT NOG / ONLY GAME
                return { game2 }
            case 3:
                let game3 = new Object();
                ...
                // SET OR CHANGE VALUE FROM USER INPUT 
                if (peu.extra || peu.extra === 0) game3.point = basePoint + peu.extra;
                // CHECK WITH A CONDITION - IF CURRENT NOG IS MORE THAN PREVIOUS RETURN WITH NOG OR WITHOUT NOG / ONLY GAME
                return { game3 }
        }
    } else if (roundNum == 2) {
        switch (peu.game) {
            case 4:
                ...
                // SET OR CHANGE VALUE FROM USER INPUT 
                if (peu.extra || peu.extra === 0) game4.point = basePoint + peu.extra;
                // CHECK WITH A CONDITION - IF CURRENT NOG IS MORE THAN PREVIOUS RETURN WITH NOG OR WITHOUT NOG / ONLY GAME
                return { game4 }
            case 5:
                let game5 = new Object();
                ...
                // SET OR CHANGE VALUE FROM USER INPUT 
                if (peu.extra || peu.extra === 0) game5.point = basePoint + peu.extra;
                // CHECK WITH A CONDITION - IF CURRENT NOG IS MORE THAN PREVIOUS RETURN WITH NOG OR WITHOUT NOG / ONLY GAME
                return { game5 }
            case 6:
                let game6 = new Object();
                ...
                // SET OR CHANGE VALUE FROM USER INPUT 
                if (peu.extra || peu.extra === 0) game6.point = basePoint + peu.extra;
                // CHECK WITH A CONDITION - IF CURRENT NOG IS MORE THAN PREVIOUS RETURN WITH NOG OR WITHOUT NOG / ONLY GAME
                return { game6 }
        }
    } else if (roundNum == 3) {
        switch (peu.game) {
            case 7:
                ...
                // SET OR CHANGE VALUE FROM USER INPUT 
                if (peu.extra || peu.extra === 0) game7.point = basePoint + peu.extra;
                // CHECK WITH A CONDITION - IF CURRENT NOG IS MORE THAN PREVIOUS RETURN WITH NOG OR WITHOUT NOG / ONLY GAME
                return { game7 }
            case 8:
                let game8 = new Object();
            ...
            case 9:
                ...
                // SET OR CHANGE VALUE FROM USER INPUT 
                if (peu.extra || peu.extra === 0) game9.point = basePoint + peu.extra;
                // CHECK WITH A CONDITION - IF CURRENT NOG IS MORE THAN PREVIOUS RETURN WITH NOG OR WITHOUT NOG / ONLY GAME
                return { game9 }
        }
    } else if (roundNum == 4) {
        switch (peu.game) {
            case 10:
                ...
                // SET OR CHANGE VALUE FROM USER INPUT 
                if (peu.extra || peu.extra === 0) game10.point = basePoint + peu.extra;
                // CHECK WITH A CONDITION - IF CURRENT NOG IS MORE THAN PREVIOUS RETURN WITH NOG OR WITHOUT NOG / ONLY GAME
                return { game10 }
            case 11:
                let game11 = new Object();
                ...
                // SET OR CHANGE VALUE FROM USER INPUT 
                if (peu.extra || peu.extra === 0) game11.point = basePoint + peu.extra;
                // CHECK WITH A CONDITION - IF CURRENT NOG IS MORE THAN PREVIOUS RETURN WITH NOG OR WITHOUT NOG / ONLY GAME
                return { game11 }
            ...
            case 12:
                ...
                // SET OR CHANGE VALUE FROM USER INPUT 
                if (peu.extra || peu.extra === 0) game12.point = basePoint + peu.extra;
                // CHECK WITH A CONDITION - IF CURRENT NOG IS MORE THAN PREVIOUS RETURN WITH NOG OR WITHOUT NOG / ONLY GAME
                return { game12 }
        }
    } else if (roundNum == 5) {
        switch (peu.game) {
            case 13:
                ...
                // SET OR CHANGE VALUE FROM USER INPUT 
                if (peu.extra || peu.extra === 0) game13.point = basePoint + peu.extra;
                // CHECK WITH A CONDITION - IF CURRENT NOG IS MORE THAN PREVIOUS RETURN WITH NOG OR WITHOUT NOG / ONLY GAME
                return { game13 }
            case 14:
                let game14 = new Object();
                ...
                // SET OR CHANGE VALUE FROM USER INPUT 
                if (peu.extra || peu.extra === 0) game14.point = basePoint + peu.extra;
                // CHECK WITH A CONDITION - IF CURRENT NOG IS MORE THAN PREVIOUS RETURN WITH NOG OR WITHOUT NOG / ONLY GAME
                return { game14 }
            case 15:
                let game15 = new Object();
                ...
                // SET OR CHANGE VALUE FROM USER INPUT 
                if (peu.extra || peu.extra === 0) game15.point = basePoint + peu.extra;
                // CHECK WITH A CONDITION - IF CURRENT NOG IS MORE THAN PREVIOUS RETURN WITH NOG OR WITHOUT NOG / ONLY GAME
                return { game15 }
        }
    }
...
module.exports.updatedPerformance = (ut, roundNum, score, tp, tpd, netID, doc) => {
    if (roundNum == 1) {
        switch (ut.game) {
            case 1:
                let game1 = new Object();
